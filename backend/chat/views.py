...
def extract_text_from_pdf(pdf_path):
    ...
    parameters:
    pdf_path(string): Path of the pdf
...
def load_image(inputs):
    """
    Loads the image and convert the image to bas64 string

    parameters:
    inputs(dic): contains the image url
...
def image_model(inputs: dict):
    ...
    parameters:
    inputs(dic): contains the image in base64 and prompt
...
def get_image_information(image_path):
    ...
    """
    vision_prompt = """
    Please extract and provide all details from the given image of a document, ensuring that the social security number is obscured as follows: [example: XXX-XX-1234].
    """
